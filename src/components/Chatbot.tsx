...
export function Chatbot({
  ...
  const handleSend = async () => {
    if (!input.trim() || !vectorStoreId || messageCount >= MAX_QUESTIONS)
      return;
    ...
    try {
      ...
      if (res.ok) {
        if (
          data.functionCall &&
          data.functionCall.name === "update_json_value"
        ) {
      ...
      <AnimatePresence mode="wait">
        {isOpen ? (
          ...
          >
            ...
            <div className="bg-[var(--color-primary)] px-4 py-3 flex items-center justify-between text-white shadow-sm shrink-0">
              ...
              <button
                ...
                className="hover:bg-white/20 rounded-full p-1 transition-colors focus-visible:outline-none focus-visible:ring-2 focus-visible:ring-white"
              >
                <span
                  aria-hidden="true"
            ...
            <div className="flex-1 overflow-y-auto p-4 space-y-4 bg-slate-50 dark:bg-slate-950/50 text-sm custom-scrollbar">
              {messages.length === 0 && (
                ...
                >
                  <span className="material-symbols-outlined text-4xl opacity-50 block mb-2">
                    smart_toy
                  </span>
                  <p>
                    Ask me questions about your document or extracted values!
                  </p>
                </motion.div>
              )}
              {messages.map((msg, idx) => (
                ...
                >
                  <div
                    className={`max-w-[85%] rounded-2xl px-4 py-2 shadow-sm whitespace-pre-wrap ${msg.role === "user" ? "bg-[var(--color-primary)] text-white" : "bg-white dark:bg-slate-800 text-slate-800 dark:text-slate-200 border border-slate-100 dark:border-slate-700"}`}
                  >
                    {msg.content}
                  </div>
                </motion.div>
            ...
            <div className="p-3 bg-[var(--color-surface-light)] dark:bg-[var(--color-surface-dark)] border-t border-slate-100 dark:border-slate-800 shrink-0">
              {!vectorStoreId ? (
                <p className="text-xs text-red-500 text-center py-2">
                  Please upload a document first to use the chatbot.
                </p>
              ) : messageCount >= MAX_QUESTIONS ? (
                <p className="text-xs text-amber-600 dark:text-amber-500 text-center py-2 font-medium">
                  Session limit reached ({MAX_QUESTIONS}/{MAX_QUESTIONS}{" "}
                  questions). Please restart the application.
                </p>
              ) : (
                <div className="flex flex-col gap-2">
                  <div className="flex items-center gap-2">
                    ...
                    <motion.button
                      ...
                      className="bg-[var(--color-primary)] text-white h-10 w-10 rounded-full flex items-center justify-center hover:bg-[var(--color-primary-dark)] transition-colors disabled:opacity-50 shrink-0 shadow-md focus-visible:outline-none focus-visible:ring-2 focus-visible:ring-[var(--color-primary)] focus-visible:ring-offset-2 dark:focus-visible:ring-offset-slate-900"
                    >
                      <span
                        aria-hidden="true"
          ...
          <motion.button
            ...
            className="h-14 w-14 bg-[var(--color-primary)] hover:bg-[var(--color-primary-dark)] text-white rounded-full shadow-[var(--shadow-elevated)] flex items-center justify-center focus-visible:outline-none focus-visible:ring-2 focus-visible:ring-[var(--color-primary)] focus-visible:ring-offset-2 dark:focus-visible:ring-offset-slate-900"
          >
            <span
              aria-hidden="true"
