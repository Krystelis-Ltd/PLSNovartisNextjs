  ...
  try {
    ...
    if (!parsedData) {
      return NextResponse.json(
        { error: "Missing parsedData for generation" },
        { status: 400 },
      );
    }
    ...
    const getChartData = (placeholderInfo: unknown) => {
      ...
      if (
        ...
        typeof val === "object" &&
        !Array.isArray(val) &&
        Object.keys(val as Record<string, unknown>).length === 1
      ) {
        val = (val as Record<string, unknown>)[
          Object.keys(val as Record<string, unknown>)[0]
        ];
      }
      ...
      if (Array.isArray(val)) {
        ...
        if (
          val.length > 0 &&
          typeof val[0] === "object" &&
          ("data" in val[0] || "chart_data" in val[0] || "labels" in val[0])
        ) {
          return val;
        }
        return null;
      ...
      if (val && typeof val === "object") {
        ...
        if (rec.chart_data && Array.isArray(rec.chart_data))
          return rec.chart_data;
        if (
          rec["Key secondary endpoint results"] &&
          Array.isArray(rec["Key secondary endpoint results"])
        )
          return rec["Key secondary endpoint results"];
      }
      return null;
    };
    ...
    const endpointData =
      getChartData(parsedData.efficacy_primary_endpoint_results_conclusion) ||
      getChartData(parsedData.primary_endpoint);

    console.log(
      "PPTX GENERATION: endpointData found?",
      !!endpointData,
      "Type:",
      typeof endpointData,
      "IsArray:",
    ...
    if (endpointData) {
      ...
      for (const chartInfo of endpointData) {
        ...
        if (labels.length > 0 && datasets.length > 0) {
          ...
          console.log(
            "PPTX GENERATION: Adding chart type:",
            pptxChartType,
            "with data:",
            JSON.stringify(chartDataObj),
          );
          ...
          try {
            slide.addChart(pptxChartType, chartDataObj, {
              ...
              dataLabelFormatCode:
                pptxChartType === pres.ChartType.pie ? "0%" : "General",
            });
          } catch (err) {
    ...
    const getTrialNumber = (data: unknown) => {
      ...
      if (typeof tn === "string")
        return tn.trim().replace(/[^a-zA-Z0-9_\-\s]/g, "");
      if (typeof tn === "object" && Object.keys(tn).length === 1) {
        const val = (tn as Record<string, unknown>)[
          Object.keys(tn as Record<string, unknown>)[0]
        ];
        if (typeof val === "string")
          return val.trim().replace(/[^a-zA-Z0-9_\-\s]/g, "");
      }
      return "Unknown_Trial";
    };
    const trialNumber = getTrialNumber(parsedData);
    const mapProfileName =
      mappingName === "results_PLS" ? "Results" : "Protocol";
    ...
    const templateFilename =
      mappingName === "protocol_PLS" ? "template2.docx" : "template.docx";
    ...
    try {
      ...
      throw new Error(
        `Template file ${templateFilename} not found at ${templatePath}`,
      );
    }
    ...
    for (const [key, rawValue] of Object.entries(parsedData)) {
      ...
      if (
        ...
        if (
          key === "treatment" ||
          (typeof value === "object" &&
            (value as Record<string, unknown>).treatment_summary &&
            (value as Record<string, unknown>).groups)
        ) {
          // It's a treatment payload
          const paragraphs: Paragraph[] = [];
          const summary =
            ((value as Record<string, unknown>).treatment_summary as string) ||
            "";
          const groups =
            ((value as Record<string, unknown>).groups as Array<{
              name: string;
              participants: string | number;
            }>) || [];
          const total =
            ((value as Record<string, unknown>).total_participants as number) ||
            0;
          ...
          if (groups.length <= 4) {
            if (groups.length > 0) {
              const runs = [
                new TextRun(
                  `${total} participants received one of these treatments:`,
                ),
              ];
              for (const g of groups) {
                runs.push(
                  new TextRun({
                    text: `• ${g.name} – ${g.participants} participants`,
                    break: 1,
                  }),
                );
              }
            ...
            const tableRows = [
              new TableRow({
                children: groups.map(
                  (
                    g: { name: string; participants: string | number },
                    i: number,
                  ) =>
                    new TableCell({
              ...
              new TableRow({
                children: groups.map((g: TreatmentGroup) => {
                  const doseMatch =
                    /(\d+\s*(?:mg|µg|mcg|g|ml|mL|IU|units?)(?:\/\w+)?)/i.exec(
                      g.name,
                    );
          ...
          key === "efficacy_primary_endpoint_results_conclusion" ||
          (value &&
            typeof value === "object" &&
            Array.isArray((value as Record<string, unknown>).chart_data))
        ) {
          let chart_data_items: ChartEndpointItem[] = Array.isArray(value)
            ? (value as ChartEndpointItem[])
            : ((value as Record<string, unknown>)
                .chart_data as ChartEndpointItem[]);
          ...
          for (const item of chart_data_items) {
            ...
            if (conclusion)
              rootBlocks.push(
                new Paragraph({
                  children: [new TextRun({ text: conclusion, italics: true })],
                }),
              );
            if (definition)
              rootBlocks.push(
                new Paragraph({ children: [new TextRun(definition)] }),
              );
            if (assessment)
              rootBlocks.push(
            ...
            const dataConfig: { labels?: string[]; datasets?: ChartDataset[] } =
              item.data || {};
            const labels = dataConfig.labels || [];
            const datasets = dataConfig.datasets || [];
            ...
            if (labels.length > 0 && datasets.length > 0) {
                        ...
                        return new TableCell({
                          ...
                          children: [
                            new Paragraph({
                              alignment: AlignmentType.CENTER,
                              children: [
                                new TextRun({ text: String(val), bold: true }),
                              ],
                            }),
                          ],
                        });
              ...
              rootBlocks.push(
                new Paragraph({ children: [new TextRun(resultsStr)] }),
              );
            }
          ...
          for (let i = 0; i < value.length; i++) {
            ...
            if (typeof item === "string") {
              runs.push(
                new TextRun({ text: `• ${item}`, break: i > 0 ? 1 : 0 }),
              );
              ...
              if (item.question && item.answer) {
                runs.push(
                  new TextRun({
                    ...
                    bold: true,
                    break: runs.length > 0 ? 2 : 0,
                  }),
                );
                runs.push(new TextRun({ text: String(item.answer), break: 1 }));
              } else if (
                item.question &&
                item.primary_endpoint_results_conclusion
              ) {
                runs.push(
                  new TextRun({
                    text: String(item.question),
                ...
                runs.push(
                  new TextRun({
                    text: String(item.primary_endpoint_results_conclusion),
                    break: 1,
                  }),
                );
              }
