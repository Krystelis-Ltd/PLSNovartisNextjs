...
export async function POST(request: NextRequest) {
  ...
  try {
    ...
    console.log(
      `[upload] Starting upload of ${files.length} files to OpenAI storage`,
    );
    ...
    const uploadPromises = files.map(async (file) => {
      try {
        ...
        console.log(
          `[upload] Uploaded: ${file.name} (${fileUploadResponse.id})`,
        );
        auditLog({
          request,
          action: "FILE_UPLOAD",
    ...
    try {
      ...
      console.log(
        `[upload] Attaching files to vector store ${vectorStoreId}...`,
      );
