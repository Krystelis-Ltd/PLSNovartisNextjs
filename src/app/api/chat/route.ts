...
export async function POST(request: NextRequest) {
  ...
  try {
    ...
    const conversationText = messages
      ...
      .join("\n");
    const inputStr =
      "Please answer the latest user request based on the following conversation history:\n" +
      conversationText;
    ...
    try {
      const response = await (
        ...
        tools: [
          ...
          },
          {
            ...
            description:
              "Updates a specific key in the extracted JSON document dataset with new information based on the user's request.",
            parameters: {
              type: "object",
              properties: {
                key: {
                  type: "string",
                  description:
                    "The exact root-level key in the JSON object to update.",
                },
                newValue: {
                  type: "object",
                  description:
                    "The complete, properly formatted new JSON object or string value to replace the existing value under the key.",
                },
              },
      ...
      const toolCall = outputArray.find(
        (item) =>
          item.type === "function_call" && item.name === "update_json_value",
      );
      ...
      console.warn(
        "[chat] responses.create failed, falling back to beta thread:",
    ...
    if (run.status === "completed") {
      ...
      const lastMsg = messagesList.data.filter(
        (m) => m.role === "assistant",
      )[0];
