...
export default function Dashboard() {
  ...
  const generateReport = async () => {
    ...
    try {
      const res = await fetch("/api/generate", {
        ...
        headers: { "Content-Type": "application/json" },
        body: JSON.stringify({
          parsedData: currentFetchedAnswers,
          mappingName,
        }),
      });
      ...
      if (res.ok) {
        ...
        const totalTimeSeconds = uploader.uploadStartTimeRef.current
          ? Math.round(
              (Date.now() - uploader.uploadStartTimeRef.current) / 1000,
            )
          : null;
  ...
  const currentFetchedAnswers = useMemo(() => {
    return pipeline.extractionFeed
      ...
      .reduce((acc: Record<string, any>, feed) => {
        ...
        if (keyIndex !== -1) {
          const m = mapping[
            String(keyIndex + 1) as keyof typeof mapping
          ] as any;
          if (m) {
            if (m.placeholder) finalKey = m.placeholder;
            else if (m.table_placeholder)
              finalKey = m.table_placeholder
                .replace(/^{{/, "")
                .replace(/}}$/, "");
          }
        }
  ...
  const handleChatbotUpdate = (keyToUpdate: string, newValue: any) => {
    pipeline.setExtractionFeed((prev) =>
      prev.map((feed) => {
        const keyIndex = keys.indexOf(feed.title);
        let finalKey = feed.title;
        if (keyIndex !== -1) {
  ...
  return (
    <div className="flex flex-col h-full w-full relative gradient-mesh">
              ...
              <div className="space-y-3">
                ...
                <div>
                  ...
                  >
                    ...
                    <option value="Non-technical Healthcare Professional">
                      Non-technical Healthcare Professional
                    </option>
                  </select>
                </div>
              </div>
            ...
            <div className="space-y-4">
              ...
              {uploader.queuedFiles.length > 0 && (
                ...
                >
                  ...
                  {uploader.isUploading
                    ? "Uploading..."
                    : `Upload ${uploader.queuedFiles.length} File${uploader.queuedFiles.length > 1 ? "s" : ""} to Vector Store`}
                </button>
              )}
              ...
              <motion.button
                ...
                disabled={
                  uploader.isUploading ||
                  !uploader.vectorStoreId ||
                  pipeline.isExtracting ||
                  uploader.queuedFiles.length > 0
                }
            ...
            <div className="space-y-3">
              ...
              <div className="space-y-1.5 max-h-64 overflow-y-auto pr-2 custom-scrollbar">
                {keys.map((key) => (
                  <label
                    key={key}
                    className="flex items-center justify-between p-2.5 rounded-lg bg-slate-50 dark:bg-slate-800 cursor-pointer border border-transparent hover:border-[var(--color-primary)]/20 group"
                  >
                    <span className="text-[13px] font-medium text-slate-700 dark:text-slate-300 truncate pr-2">
                      {key.replace(/_/g, " ")}
                    </span>
                    <input
              ...
              <div className="space-y-4 border-t border-slate-200 dark:border-slate-800 pt-6">
                ...
                <div className="space-y-2">
                  {uploader.queuedFiles.map((file, idx) => (
                    <div
                      key={`q-${idx}`}
                      className={`flex items-center gap-3 p-3 rounded-lg border border-dashed border-amber-300 dark:border-amber-700/50 bg-amber-50/50 dark:bg-amber-900/10`}
                    >
                      ...
                      <button
                        onClick={() => uploader.removeQueuedFile(file.name)}
                  ...
                  {uploader.files.map((file, idx) => (
                    <div
                      key={`f-${idx}`}
                      className={`flex items-center gap-3 p-3 rounded-lg border border-slate-200 dark:border-slate-800 ${file.opacity}`}
                    >
                      ...
                      <span
                        className={`material-symbols-outlined ${file.statusColor} text-xl`}
                      >
                        {file.statusIcon}
                      </span>
                    </div>
          ...
          <div className="p-4 border-b border-slate-200 dark:border-slate-800 bg-white dark:bg-slate-900 shrink-0 flex flex-col gap-3 shadow-sm z-10">
            <div className="flex justify-between items-center">
              ...
              <div className="flex items-center gap-3">
                ...
                <span className="text-xs font-medium px-2 py-0.5 rounded-full bg-[var(--color-primary)]/10 text-[var(--color-primary)] italic">
            ...
            {/* Progress Bar */}
            {(pipeline.isExtracting ||
              (pipeline.extractionProgress === 100 &&
              ...
              {pipeline.extractionFeed.length === 0 ? (
                ...
                >
                  ...
                  <p className="text-sm font-medium">
                    Run AI Extraction to begin building the document.
                  </p>
                </motion.div>
              ) : (
                pipeline.extractionFeed.map((feed, idx) => (
                  <ExtractionCard
                    ...
                    onSetRefineInstructions={(key, val) =>
                      pipeline.setRefineInstructions((prev) => ({
                        ...prev,
                        [key]: val,
                      }))
                    }
      ...
      <AnimatePresence>
        {showTestUI && <PipelineTestUI onClose={() => setShowTestUI(false)} />}
      </AnimatePresence>
