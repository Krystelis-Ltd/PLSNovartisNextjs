...
export function auditLog({
  ...
  const fallbackId = `fb-${Date.now()}-${fallbackCounter++}`;
  let uuid = fallbackId;
  try {
  ...
  // Securely extract IP to prevent spoofing
  let publicIp = request.headers.get("x-azure-clientip");
  if (!publicIp) {
    const xForwardedFor = request.headers.get("x-forwarded-for");
    if (xForwardedFor) {
      const ips = xForwardedFor.split(",").map((ip) => ip.trim());
      publicIp = ips[ips.length - 1]; // In Azure, the proxy appends the real IP to the end
    }
  }
  publicIp = publicIp || request.headers.get("x-real-ip") || "unknown";
